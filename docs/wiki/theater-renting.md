...
Private theaters can be rented for a limited time using the server’s point system.  
While rented, the renter is the theater owner for the full duration — even if they disconnect and rejoin.
...
4. Cost = minutes × `cinema_rent_cost_per_minute` in the active currency.

### While you are the renter

From the **owner** panel (scoreboard):
...
Admins can **Cancel rent** on a theater they do not own.  
The owner is refunded remaining time. If the owner is offline, the refund is queued and applied on their next connect.

---

## Requirements

- A private theater location on the map (`THEATER_PRIVATE`)
