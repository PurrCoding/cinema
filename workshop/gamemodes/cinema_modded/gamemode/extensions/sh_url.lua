...
-----------------------------------------------------------------------------
function escape(s)
    return string_gsub(s, "([^A-Za-z0-9_])", function(c) return string_format("%%%02x", string_byte(c)) end)
end
...
-----------------------------------------------------------------------------
function unescape(s)
    return string_gsub(s, "%%(%x%x)", function(hex) return string_char(tonumber(hex, 16)) end)
end
...
function parse2(url, default)
    ...
    if parsed.query then
        ...
        local prmarr = string_Explode("&", prmstr)
        local params = {}

        for i = 1, #prmarr do
            local tmparr = string_Explode("=", prmarr[i])
